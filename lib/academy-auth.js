...
const ACADEMY_DASHBOARD_PATH = '/academy/dashboard/';
...
export function requireAuth({ loginPath = '/academy/login/', returnParam = 'redirect' } = {}) {
  const user = getCurrentUser();
  ...
  loginUrl.searchParams.set(returnParam, next);
...
export function updateCurrentUser(updater) {
  ...
  const updatedUser = typeof updater === 'function' ? updater(session.user) : { ...session.user, ...updater };
  persistSession(updatedUser);

  const users = getStoredUsers();
  const userIndex = users.findIndex((user) => user.id === updatedUser.id || user.email === updatedUser.email);
  if (userIndex >= 0) {
    const existing = users[userIndex];
    users[userIndex] = { ...existing, ...updatedUser, passwordFingerprint: existing.passwordFingerprint };
...
export function enrollCurrentUserInCourse(courseSlug) {
...
export async function continueWithGoogleOAuth({ mode = 'sign-in' } = {}) {
  ...
  if (!clerk) {
    return {
      ...
      message: 'Connexion Google prête côté UI. Installez et initialisez Clerk avec NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY pour activer OAuth Google.'
    };
  }
  ...
  return {
    ...
    message: 'Clerk est détecté, mais l’instance chargée ne fournit pas encore de méthode OAuth compatible Google.'
  };
}
...
export function getRedirectTarget(defaultTarget = '/academy/dashboard/') {
